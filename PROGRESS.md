...
`[x]` — R16, R17, R18, R19. Sub-plans 10a-10h built the original canvas/n8n rendering. Sub-plans 10i-10k (2026-08-06) brought the n8n canvas into full parity with `docs/ARCHITEQ-n8n-Canvas-Complete-Spec.md` and its own `reference/architeq-ux-mockup.html` source, verified against the real served app per DP14 (not code-reading alone, after two rounds of exactly that missing real gaps): real icons/node IDs/needs-config styling/legend/connection labels/exact layout numbers (10i-10j); connection arrowheads + animated flow-dot, a dot-grid canvas background, the three-way `Python | n8n | Tree` mode toggle (replacing the tree diagram's old always-stacked-above-the-canvas layout, correcting `ARCHITEQ-APP-FLOW.md`), and free node dragging with live connection redraw (10k).
