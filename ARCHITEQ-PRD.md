...
- **R40.** An artifact whose required source tree does not exist yet for the current domain
  (see OQ6) renders an explicit "not generated yet" state naming the specific blocking
  dependency, rather than fabricated or placeholder content — extends WD2 (No Silent
  Failure) to planning artifacts. `Resolved` per sub-plan 12b: all six artifacts now have a
  real per-domain generator (`backend/planning_artifacts.py`); PRD, TDD, and Engineering
  Plan are always derivable from a domain's frozen Workflow Tree alone, while App Flow and
  Design Brief still need that domain's own frozen Component Tree, and Backend Schema still
  needs its own frozen Data Architecture — R40's honest state is per-domain-conditional now,
  not a fixed five-of-six gap in ARCHITEQ itself.
...
6. **No real per-domain generator exists yet for 5 of DP11's 6 planning artifacts.** `Resolved` by sub-plan 12b: all six DP11 artifacts (PRD, TDD, App Flow, Design Brief, Backend Schema, Engineering Plan) now have a real, deterministic, per-domain generator (`backend/planning_artifacts.py`), each derived only from that domain's already-frozen Workflow Tree and, where relevant, its Component Tree or Data Architecture — never an LLM call, never ARCHITEQ's own repository documents used as end-user content. The Backend-Schema-equivalent renders the Data Architecture's own canonical entities directly rather than re-deriving them, so it is not redundant with the Component Tree's Attribute Enumeration; it is a distinct per-domain view of a different source (persistent data model vs. structural components). App Flow and Design Brief still require a frozen Component Tree to exist at all (to know whether the domain has any UI-tagged component); Backend Schema still requires a frozen Data Architecture. Absent either source tree, R40's honest "not generated yet" state names that specific dependency — a valid per-domain state, not an unresolved scope question about ARCHITEQ itself.
