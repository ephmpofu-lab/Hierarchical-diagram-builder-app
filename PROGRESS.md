...
`[ ]` — R41 to R50. Per `docs/ARCHITEQ-Data-Architecture-Layer-Spec.md`. A second, synchronized architectural layer alongside the Workflow Layer (never an independent ERD page, never sharing the Workflow Layer's own layout) -- derived from a domain's already-frozen Workflow Tree: every Atomic step classified by its data-persistence operation (CREATE/READ/WRITE/UPDATE/DELETE/QUERY/TRANSIENT), persistent entities deduplicated and given stable Data IDs, one canonical model driving the ERD/SQL DDL/workflow mappings together, bidirectional cross-layer traceability (domain + Atomic step id + Data ID + operation, never a bare id alone), and prominence-toggling between the two layers. Depends on Modules 1-10. Rated Complex, sequenced as sub-plans per `.agent/plans/13.data-architecture-layer.md`. Not started.
